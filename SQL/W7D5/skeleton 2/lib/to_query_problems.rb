...
#
#  id          :integer      not null, primary key
#  cat_id      :integer      not null, foriegn key  CATS --> join cattoys ON cats.id = cattoys.cat_id
#  toy_id      :integer      not null, foreign key  TOYS --> Join toys ON cattoys.toy_id = toys.id
...
def no_apples_for_blair
  ...
  execute(<<-SQL)
      SELECT c1.name
      FROM cats AS c1
      JOIN cattoys ON c1.id = cattoys.cat_id
...
def no_apples_for_blair_sub
  ...
  WHERE c1.name != 'Blair' AND toys.id IN (
      SELECT toys.id
      FROM toys
      WHERE toys.name = 'Apple')
...
def price_like_shiny_mouse
  ...
  # DO NOT USE A SUBQUERY
  execute(<<-SQL) 
  SELECT non.name, non.price
  FROM toys AS non
  JOIN toys AS shiny ON shiny.price = non.price
  WHERE shiny.name = 'Shiny Mouse' AND non.name != 'Shiny Mouse';
  SQL
end
...
def just_like_orange
  ...
  # DO NOT USE A SUBQUERY
  execute(<<-SQL)
  SELECT c2.name, c2.breed
  FROM cats AS c1
  JOIN cats AS c2 ON c1.breed = c2.breed
  WHERE c1.name = 'Orange' AND c2.name != 'Orange'
  ORDER BY c2.name;
  SQL
end
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    SELECT DISTINCT notjetcat.name, toys.name
    FROM cats

    JOIN cattoys ON cats.id = cattoys.cat_id
    JOIN toys ON toys.id = cattoys.toy_id

    JOIN cattoys AS notjetcattoys ON toys.id = notjetcattoys.toy_id
    JOIN cats AS notjetcat ON notjetcat.id = notjetcattoys.cat_id
    
    WHERE cats.name = 'Jet' AND notjetcat.name != 'Jet';
